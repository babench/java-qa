...
public class TestSuitePO extends BasePage {
    ...
    private void populateDetails(String details) {
        ...
        if (details != null && !details.isEmpty()) {
            sendKeysToContents(1, details).backToFrame(Frames.workframe);
        }
    }
    ...
    private TestSuitePO addTestCase(TestCaseEntity testCaseEntity) {
        ...
        if (testCaseEntity.getTestCaseSummary() != null && !testCaseEntity.getTestCaseSummary().isEmpty()) {
            sendKeysToContents(1, testCaseEntity.getTestCaseSummary()).backToFrame(Frames.workframe);
        }
        ...
        if (testCaseEntity.getTestCasePreconditions() != null && !testCaseEntity.getTestCasePreconditions().isEmpty()) {
            sendKeysToContents(2, testCaseEntity.getTestCasePreconditions()).backToFrame(Frames.workframe);
        }
    ...
    private TestSuitePO addStep(TestCaseStepEntity testCaseStepEntity) {
        ...
        if (testCaseStepEntity.getStepAction() != null && !testCaseStepEntity.getStepAction().isEmpty()) {
            sendKeysToContents(1, testCaseStepEntity.getStepAction()).backToFrame(Frames.workframe);
        }

        if (testCaseStepEntity.getStepExpectedResult() != null && !testCaseStepEntity.getStepExpectedResult().isEmpty()) {
            sendKeysToContents(2, testCaseStepEntity.getStepExpectedResult()).backToFrame(Frames.workframe);
        }
    ...
    private TestSuitePO backToFrame(Frames workframe) {
        switchToMainFrame();
        switchToFrame(Frames.workframe);
        return this;
    }

    /*for edit reach element*/
    private TestSuitePO sendKeysToContents(int index, String stepExpectedResult) {
        driver.switchTo().frame(driver.findElement(By.id("cke_" + index + "_contents")).findElement(By.cssSelector("iframe")));
        driver.findElement(By.cssSelector("body")).sendKeys(stepExpectedResult);
        return this;
    }
